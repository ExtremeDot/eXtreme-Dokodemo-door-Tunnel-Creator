...
01_inbounds.json: شامل پروتکل‌های dokodemo-door برای گوش دادن به پورت‌های فورواردی سرور ایران و پورت‌های mixed (ساکس/اتچمنت) کمکی.

02_outbounds.json: شامل دیتای رمزنگاری شده خروجی پروتکل‌ها به همراه آدرس‌های پشت مرز، تنظیمات جریان (StreamSettings) مانند gRPC یا WS و پورت خروجی.

03_routing.json: هدایت‌گر اصلی ترافیک؛ متصل‌کننده دقیق پورت‌های ورودی به ساب‌تانل‌های اختصاصی سرورهای خارج بدون تداخل ترافیکی.

🚀 نحوه استفاده (Quick Start)
کد فایل index.html را دانلود کنید.

آن را در مرورگر خود باز کنید (یا روی یک وب‌سرور/گیت‌هاب پیج هاست کنید).

فایل‌های JSON فعلی سرور ایران خود را (در صورت وجود) در باکس‌های مبدا وارد کنید.

وارد تب‌های سرور ۱ تا ۱۰ شوید، نام، پورت فوروارد و لینک تانل خارج را پیست کنید.

خروجی‌های آماده شده در پایین صفحه را کپی کرده و در مسیر /usr/local/etc/xray/conf.d/ سرور خود جایگزین کنید.

دستور systemctl restart xray را اجرا کنید. تانل شما با موفقیت آپدیت شد!
